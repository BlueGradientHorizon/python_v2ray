...
from python_v2ray.tester import (
    ConnectionTester,
)  # NEW: Import ConnectionTester for unified testing
...
def run_test_with_uri(
    ...
    * A helper function to test a single URI from start to finish using the ConnectionTester.
    ...
    print(
        f"* PARSING SUCCESS: Protocol='{params.protocol}', Address='{params.address}:{params.port}', Display Tag='{params.display_tag}'"
    )
    ...
    tester = ConnectionTester(
        vendor_path=vendor_path, core_engine_path=core_engine_path
    )

    # Use test_uris method which handles XrayCore lifecycle and config building
    # We pass a list of one ConfigParams object.
    results = tester.test_latency(profiles=[params], timeout=30)  # Added timeout
    ...
    * Runs a series of tests with different URI types using ConnectionTester.
    ...
    for uri in test_uris:
        if "YOUR_" in uri or "your." in uri:  # Check for placeholders
            ...
            print(
                "! Please replace it with your own real config URI in the 'test_uris' list to test it."
            )
            continue
