...
def main():
    ...
    * This uses the XrayApiClient to communicate with Xray's gRPC StatsService.
    ...
    if "YOUR_UUID" in vless_uri:
        print(
            "! Please replace the placeholder URI in the script with your own to run this demo."
        )
        return
    ...
    outbound_internal_tag = (
        params.tag
    )  # This is the cleaned-up tag for Xray's internal use
    ...
    print(
        f"* Building Xray configuration to monitor outbound with tag: '{outbound_internal_tag}' (Display: '{outbound_display_tag}')"
    )
    ...
    outbound = builder.build_outbound_from_params(
        params, explicit_tag=outbound_internal_tag
    )
    if not outbound:
    ...
    )  # Add a direct outbound for completeness
    ...
    with XrayCoreProcess(
        ...
        print(
            "* Now, generate some traffic through the SOCKS proxy (e.g., configure your browser or use curl via 127.0.0.1:10808)."
        )
        print(
            f"* Monitoring stats for outbound tag: '{outbound_display_tag}' (internal: '{outbound_internal_tag}')"
        )
        ...
        try:
            while True:
                ...
                if stats:
                    ...
                    print(
                        f"\r* Live Stats for '{outbound_display_tag}': Uplink: {uplink_mb:.2f} MB | Downlink: {downlink_mb:.2f} MB",
                        end="",
                        flush=True,
                    )
                else:
                    print(
                        f"\r* Waiting for traffic on tag '{outbound_display_tag}'...",
                        end="",
                        flush=True,
                    )
