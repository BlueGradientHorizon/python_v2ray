...
def main():
    ...
    * from a list based on their core properties, ignoring their display tags.
    ...
    parsed_configs: List[ProxyProfile] = load_configs(source=raw_uris)
    ...
    unique_configs: List[ProxyProfile] = deduplicate_configs(parsed_configs)
    ...
    print(
        "Display Tags of final unique configs:", [p.display_tag for p in unique_configs]
    )
    ...
    for config in unique_configs:
        ...
        print(
            f"- Protocol: {config.protocol:<10} | Address: {config.address}:{config.port:<5} | Display Tag: {config.display_tag}"
        )
